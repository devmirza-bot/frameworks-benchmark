...
const frameworks = [
  ...
  },
  {
    ...
    library: Vue,
    render: (data) =>
      data.map((item) => Vue.h("div", { key: item.id }, item.id)),
  },
  ...
  try {
    ...
    if (fs.existsSync("historical_log.json")) {
      historicalData = JSON.parse(
        fs.readFileSync("historical_log.json", "utf-8")
      );
    }
...
const runBenchmark = () => {
  ...
  frameworks.forEach((framework) => {
    ...
    console.log(
      `${framework.name} took ${elapsedTimeInSeconds.toFixed(
        4
      )} seconds to render.`
    );
  });
